...
const cartCount = 1; // заглушка

const Header = () => {  return (
    <header className="container-app flex justify-between w-full py-3.75 mb-7">
        ...
        <div className="flex gap-8 items-center">
            ...
            <Link to="/cart" className="relative inline-block">
                ...
                {cartCount > 0 && (<span className="absolute -top-2 -right-2 text-4.25 flex items-center justify-center text-surface bg-text-secondary size-4.25 rounded-full">{cartCount}</span>)}
            </Link>
        </div>
    </header>
  )
